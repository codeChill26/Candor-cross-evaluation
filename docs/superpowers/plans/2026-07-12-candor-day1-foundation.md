...
- Create: `components/ui/button.tsx`, `input.tsx`, `label.tsx`, `form.tsx`, `card.tsx`, `dialog.tsx`, `dropdown-menu.tsx`, `avatar.tsx`, `badge.tsx`, `separator.tsx`, `table.tsx`, `sonner.tsx` (generated)
...
```bash
npx shadcn@latest add button input label form card dialog dropdown-menu avatar badge separator table sonner --yes
```

- [ ] **Step 3: Replace default theme tokens with Candor's neutral + single accent palette**

Edit `app/globals.css` — replace the `:root` and `.dark` variable blocks (generated by shadcn init) with:
...
This is a single deliberate accent (a trustworthy blue, `hsl(221 83% 40%)`) on a neutral gray/white/black scale — no other saturated colors — matching the "văn phòng / corporate SaaS" direction in `candor-project.md` → Design Direction. `--accent` and `--primary` are intentionally the same hue so buttons/links/status all read as one consistent brand color.
...
**Interfaces:**
- Consumes: `createClient()` from `@/lib/supabase/client` (Task 5), `loginSchema`/`registerSchema` from `@/lib/validations/auth` (Task 4), shadcn `Form`/`Input`/`Button`/`Card` (Task 2).
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="email" placeholder="ban@congty.com" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
